$dir = "C:\Users\MTS\Music\Coding\MTSWebsite\2digix-business-it-solutions-html-template-2025-09-21-06-46-01-utc\digix-main"

# Helper: set nested property "a.b.c" = value in hashtable
function Set-Nested($ht, $path, $value) {
    $parts = $path -split '\.'
    $current = $ht
...
# Read current flat en.json
$flatEn = Get-Content "$dir\lang\en.json" -Raw | ConvertFrom-Json

# Build nested EN (English reference)
$enNested = @{}
# Build nested FR (French)
$frNested = @{}
# Build nested ZH (Chinese)
$zhNested = @{}

# ===== TRANSLATION MAP =====
# Keys grouped by page section
# Format: key = English / French / Chinese
...
$translations = @(
    ...
    @('about.process.step1.desc', 'Understanding your requirements and project goals through in-depth consultation.', 'Comprendre vos besoins et objectifs de projet grâce à une consultation approfondie.', '通过深入咨询了解您的需求和项目目标。'),
    ...
    @('contact.form.subject', 'Subject', 'Sujet', '主题'),
...
# Apply translations to nested structures
foreach ($t in $translations) {
    ...
    Set-Nested $enNested $key $enVal
    Set-Nested $frNested $key $frVal
    Set-Nested $zhNested $key $zhVal
}

# Check for any keys from original en.json that are missing
$missing = @()
foreach ($p in $flatEn.PSObject.Properties) {
    $key = $p.Name
    $parts = $key -split '\.'
    $current = $enNested
    $found = $true
...
if ($missing.Count -gt 0) {
    ...
    $missing | ForEach-Object { Write-Output "  $_ -> $($flatEn.$_)" }
    # Add missing ones with English values
    foreach ($m in $missing) {
        Set-Nested $enNested $m $flatEn.$m
        Set-Nested $frNested $m $flatEn.$m
        Set-Nested $zhNested $m $flatEn.$m
    }
}

# Write EN
$jsonEn = $enNested | ConvertTo-Json -Depth 10
[System.IO.File]::WriteAllText("$dir\lang\en.json", $jsonEn, [System.Text.UTF8Encoding]::new($false))

# Write FR
$jsonFr = $frNested | ConvertTo-Json -Depth 10
[System.IO.File]::WriteAllText("$dir\lang\fr.json", $jsonFr, [System.Text.UTF8Encoding]::new($false))

# Write ZH
$jsonZh = $zhNested | ConvertTo-Json -Depth 10
[System.IO.File]::WriteAllText("$dir\lang\zh.json", $jsonZh, [System.Text.UTF8Encoding]::new($false))

Write-Output "Done! EN: $($enNested.Count) sections, FR: $($frNested.Count) sections, ZH: $($zhNested.Count) sections"
Write-Output "Missing original keys not translated: $($missing.Count)"
